...
## 1. 対象環境と事前のソフトウェアインストール

動作環境はWindows, Mac, Linuxの全てで動作する．
また，本プログラム群はPython, Pythonのウィンドウシステム用のライブラリとSQLite3を利用しているため，
これらをインストールする必要があります．

### 1.1. 事前にインストールする必要があるソフトウェア
最初にPythonおよびSQLite3をインストールしてください．

### 1.2. インストールが必要なPythonライブラリ
このプログラム群では，以下のライブラリを使っている．
- sqlite3
- csv
- flet_multi_page
- flet
...
アップロードするためのもので，``anbient``は「IoT可視化サービス([https://ambidata.io/](https://ambidata.io/))」にアップロードするための
